...
void bubbleSortArray(int array[], int elems)
{
  ...
  int temp;
  int bottom = elems - 1;
  // bottom indicates the end part of the array
  // where the largest values have settled in order
  ...
  do
    {
      ...
      for (int count = 0; count < bottom; count++)
      {
        ...
        { // the next three lines do the swap
          temp = array[count];
      ...
      bottom--;
      // bottom is decremented by 1
      // since each pass through the array adds one more value that is set in order.
    } while (swap != false);
    // loop repeats
    // until a pass through the array
    // with no swaps occurred.
}